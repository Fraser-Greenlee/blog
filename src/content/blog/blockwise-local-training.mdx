...
None of these runs is trained to the point of diminishing returns. Compute-optimal is roughly twenty tokens per parameter; my best-trained run saw under two. So a skeptic has a clean objection: maybe the local method only helps while a model is under-trained, and bigger models in my sweep are simply more under-trained. The advantage would then be a mirage that vanishes at convergence.
